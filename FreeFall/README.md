...
## Alternate Model
This model uses complex equations to calulate acceleration, velocity, and position over time. This solution is great for ensuring that values should be accurate and limits the posibility of compounded inaccuracies in the data. 

That being said, I would like to try another model that approximates the values for acceleration, velocity, and position based on small deltas in time and approximating integration using an interative method. This approach could be vulnerable to inaccuracies, particularly with slower computers where the time deltas were longer between game loop updates. 
